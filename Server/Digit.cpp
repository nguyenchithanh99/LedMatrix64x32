...
Digit::Digit(PxMATRIX* d, byte value, uint16_t xo, uint16_t yo, int h, int w, uint16_t color) {
  _display = d;
  _value = value;
  xOffset = xo;
  yOffset = yo;
...
void Digit::DrawColon(uint16_t c)
{
  ...
  drawFillRect(-4, segHeight-1, 2,2, c);
  drawFillRect(-4, segHeight+1+3, 2,2, c);
}
